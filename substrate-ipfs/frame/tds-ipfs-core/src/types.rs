...
pub struct OffchainData {
	pub data: Vec<u8>,
	pub meta_data: Vec<u8>, // can add further data such as enums here
}

impl OffchainData {
	/// creates a new instance with the given data
	pub fn new(data: Vec<u8>, meta_data: Vec<u8>) -> OffchainData {
		OffchainData { data, meta_data }
	}
}

/// Wrapper class containing infos of a file within IPFS which had been uploaded by the substrate
/// node
#[derive(Debug, Clone, Encode, Decode, PartialEq, TypeInfo, Default)]
#[cfg_attr(feature = "std", derive(Deserialize))]
pub struct IpfsFile {
