...
/// * `is_in_offchain_context` - Tells the storage, if the method call is within onchain or offchain
///   context. That is important, since in both states use different storage APIs and storing fails,
///   it the flag is wrong

pub fn store_cid_data_for_values<T: Config>(
	block_number: T::BlockNumber,
	cid: Vec<u8>,
	meta_data: Vec<u8>,
	is_in_offchain_context: bool,
) {
...
///   context. That is important, since in both states use different storage APIs and storing fails,
///   it the flag is wrong
pub fn store_cid_data<T: Config>(
	block_number: T::BlockNumber,
...
///   context. That is important, since in both states use different storage APIs and storing fails,
///   it the flag is wrong
pub fn store_cid_data_for_key<T: Config>(
	key: &Vec<u8>,
	cid_data: &OffchainData,
	is_in_offchain_context: bool,
) {
...
pub fn read_cid_for_block_number<T: Config>(
	block_number: T::BlockNumber,
) -> Result<OffchainData, StorageRetrievalError> {
...
///
/// The OffchainStorageData is a wrapper class around the stored data. You can access the actual
/// storage data through OffchainStorageData.data
pub fn read_cid_data_for_block_number<T: Config>(
	block_number: T::BlockNumber,
) -> Result<Option<OffchainData>, StorageRetrievalError> {
	let key = offchain_data_key::<T>(block_number);
...
/// The OffchainStorageData is a wrapper class around the stored data. You can access the actual
/// storage data through OffchainStorageData.data
pub fn offchain_storage_data_for_key<T: Decode>(
	key: &Vec<u8>,
) -> Result<Option<T>, StorageRetrievalError> {
	let storage_ref = StorageValueRef::persistent(key);
